...
app = Flask(__name__)
app.debug = True
app.config['SECRET_KEY'] = 'jfhqoehfoijfowijfeoiajfeojeoifjoafijoiaehfiuashfbFHLAHFAHDKFUilhfjiwfhbiKFVGSCVjgvdqw78380739uqhbasdUVCC'
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:icedout@localhost:5432/songsdb"
...
playlist_song_collections = db.Table('playlist_song_collections',db.Column('song_id',db.Integer, db.ForeignKey('songs.id')),db.Column('playlist_id',db.Integer, db.ForeignKey('playlists.id')))
...
class Song(db.Model):
    ...
    rels = db.relationship("Playlist", backref="Song")
...
class Playlist(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    playlist_names = db.Column(db.String(100))
    songs = db.relationship("Song", secondary=playlist_song_collections, backref=db.backref('playlists', lazy='dynamic'), lazy='dynamic')
    relp = db.relationship("Song", backref="Playlist")
...
class SongForm(FlaskForm):
    ...
    genre = StringField("What is the genre of that song?", validators=[Required()])
    album = StringField("What is the album this song is on?", validators=[Required()])
