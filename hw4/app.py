...
import requests
import json
from giphy_api_key import api_key, uri
...
app.config['SECRET_KEY'] = '28uehdqw-'
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://{}/chalseoHW4db".format(uri)  # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',db.Column('searches_id',db.Integer, db.ForeignKey('searches.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
user_collection = db.Table('user_collection',db.Column('personalgifcollectionsn_id',db.Integer, db.ForeignKey('personalgifcollections.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
class User(UserMixin, db.Model):
    ...
    collection_id = db.relationship('PersonalGifCollection', backref="User")
...
def load_user(user_id):
    return User.query.get(int(user_id)) # returns Usr object or None
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return "Gif ---> title: {0} url: {1}".format(self.title, self.embedURL)
...
def get_gifs_from_giphy(search_string):
    ...
    baseurl = "https://api.giphy.com/v1/gifs/search?q={0}&api_key={1}&limit=5".format(search_string.replace(" ", "+"), api_key)
    results = requests.get(baseurl).json()['data']
    r = []
    for re in results:
        r.append(re)
    return r
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        search_term = SearchTerm(term=term, gifs=[])
        search_term.gifs = []
        for gif in get_gifs_from_giphy(term):
            tmp_gif = get_or_create_gif(title=gif['title'], url=gif['embed_url'])
            search_term.gifs.append(tmp_gif)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    c = PersonalGifCollection.query.filter_by(name=name, user_id=current_user.id).first()
    if c:
        return c
    else:
        c = PersonalGifCollection(name=name, user_id=current_user.id, gifs=[])
        for g in gif_list:
            c.gifs.append(g)
        db.session.add(c)
        db.session.commit()
    ...
    # TODO 364: This function should get or create a personal gif collection.
        ...
        # In other words, based on the input to this function, if there exists a collection with the input name, associated with the current user,
        #then this function should return that PersonalGifCollection instance.

    # However, if no such collection exists, a new PersonalGifCollection instance should be created,
        # and each Gif in the gif_list input should be appended to it
        # (remember, there exists a many to many relationship between Gifs and PersonalGifCollections).
...
def index():
    form = GifSearchForm()
    if form.validate_on_submit():
        t = get_or_create_search_term(form.search.data)
        return redirect(url_for('search_results', search_term=t.term))
...
def search_terms():
    q = SearchTerm.query.all()
    return render_template('search_terms.html', all_terms=q)
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        r = []
        for g in gif_picks:
            index = choices[int(g)-1][0]
            r.append(get_gif_by_id(index))
        get_or_create_collection(name, current_user, r)
...
def collections():
    ...
    # TODO 364: This view function should render the collections.html template so that only the current user's personal
        # gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
