...
collections = db.Table('collections',db.Column('collection_id',db.Integer, db.ForeignKey('collections.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
searches = db.Table('searches',db.Column('searches_id',db.Integer, db.ForeignKey('searches.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
class User(UserMixin, db.Model):
    ...
    rel = db.relationship('PersonalGifCollection', secondary=collections, backref="User")
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        for g in gifs:
             tmp = get_or_create_gif(title=g['title'], embedURL=g['url']['embed_url'])
             search_term.id = search_term.append(tmp.id)
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the
    # return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search
    # terms and gifs, allowing you to do this!).
    ...
    # HINT: I recommend using print statements as you work through building this function and use it in invocations in
    # view functions to ensure it works as you expect!

def get_or_create_collection(name, current_user, gif_list=[]):
    """Always returns a PersonalGifCollection instance"""
    collection = db.session.query(PersonalGifCollection).filter_by(name=name, id=current_user).first()
    if collection:
        return collection
    else:
        #more gif iteration needs to be done here
        collection = PersonalGifCollection(name=name, id=current_user)
